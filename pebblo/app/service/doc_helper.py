...
class LoaderHelper:
        ...
        top_n_findings = [
            {
                "fileName": key,
                "fileOwner": "-"
                if value.get("fileOwner", "-") is None
                else value.get("fileOwner", "-"),
                "sourceSize": 0
                if value.get("sourceSize", 0) is None
                else value.get("sourceSize", 0),
            ...
            data_source_findings = [
                {
                    key: value[key]
                    for key in value
                    if key != value[key] and key != "unique_snippets"
                }
    ...
    def _get_load_history(self):
        ...
        top_n_latest_loader_id = load_ids[
            -ReportConstants.loader_history_limit.value - 1 :
        ]
        ...
        if (
            len(load_history["history"]) == ReportConstants.loader_history_limit.value
    ...
    def _get_doc_report_metadata(self, doc, raw_data):
        ...
        (
            ...
            file_count,
            data_source_findings,
        ) = self._fetch_variables(raw_data)
    ...
    def process_docs_and_generate_report(self):
        ...
        logger.debug(
            "Iterating input doc list and perform classification and aggregating report data"
        )
